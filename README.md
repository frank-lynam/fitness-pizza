...
**Current**: v2.9.18
- **Dashboard + food library UX** — bulk/cut progress bar highlight now shows actual week-over-week weight change (blue for gain, red for loss) instead of the goal delta; food library "Use" popup now includes an amount slider (per-100g foods: 0.01–3.00 in 0.01 steps; all others: 0.5–5 in 0.5 steps) that updates the entry in real-time; toast fades on normal schedule, or 4s after the last slider move; fix updater guard that permanently blocked re-downloading a bundle after a failed restart

**v2.9.17**
- **Dashboard + food library UX** — same features (bundle stuck due to updater guard bug)
